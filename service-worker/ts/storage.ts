...
import { cbSettingsClient } from "./index.js";
import type {
...
import type {
	KeyValueMap,
	OldStorageObject,
	SettingsStorageObject,
	StorageObject,
} from "./interfaces/storage.js";

const defaultStorage: StorageObject = {
	version: "0",
	blockedChannels: [],
...
export async function loadDataFromStorage() {
	...
	appState.buttonVisible = storageObject.settings.buttonVisible;
	appState.buttonColor = storageObject.settings.buttonColor;
	appState.buttonSize = storageObject.settings.buttonSize;
	appState.animationSpeed = storageObject.settings.animationSpeed;
...
export function handleAddBlockingRuleMessage(message: AddBlockingRuleMessage) {
	...
	if (message.content.blockingChannelRegExp !== undefined) {
		blockedChannelsRegExp[message.content.blockingChannelRegExp] = message
	...
	if (message.content.blockingCommentRegExp !== undefined) {
		blockedComments[message.content.blockingCommentRegExp] = message.content
			.caseInsensitive
			? "i"
			: "";
	...
	if (message.content.blockingVideoTitleRegExp !== undefined) {
		blockedVideoTitles[message.content.blockingVideoTitleRegExp] = message
			.content.caseInsensitive
			? "i"
			: "";
 ...
 */
export function handleRemoveBlockingRuleMessage(
...
function convertOldStorage() {
	...
	chrome.storage.local.get(defaultOldStorage).then((result) => {
		...
		// Add settings
		const settingsStorageObject: SettingsStorageObject = {
