import { cbSettingsClient } from "./Client.js";
...
const blockedChannelsSelect: HTMLSelectElement = document.getElementById(
	"blocked-channels",
) as HTMLSelectElement;
const blockedChannelsInput: HTMLInputElement = document.getElementById(
	"blocked-channels-input",
) as HTMLInputElement;
const blockedChannelsAddBtn: HTMLButtonElement = document.getElementById(
	"blocked-channels-add-btn",
) as HTMLButtonElement;
const blockedChannelsRemoveBtn: HTMLButtonElement = document.getElementById(
	"blocked-channels-remove-btn",
) as HTMLButtonElement;
const caseInsensitiveRow = document.getElementById(
	"case-insensitive-row",
) as HTMLDivElement;
const caseInsensitiveCheckbox = document.getElementById(
	"case-insensitive-checkbox",
) as HTMLInputElement;
const nav = document.getElementById("main-nav") as HTMLElement;

const headingElement: HTMLHeadingElement = document.getElementById(
	"heading",
) as HTMLHeadingElement;
const rulesSection: HTMLDivElement = document.getElementById(
	"rules-section",
) as HTMLDivElement;
const appearanceSection: HTMLDivElement = document.getElementById(
	"appearance-section",
) as HTMLDivElement;
const importExportSection: HTMLDivElement = document.getElementById(
	"import-export-section",
) as HTMLDivElement;
const aboutSection: HTMLDivElement = document.getElementById(
	"about-section",
) as HTMLDivElement;
const faqSection: HTMLDivElement = document.getElementById(
	"faq-section",
) as HTMLDivElement;

const blockedChannelsNav: HTMLLIElement = document.getElementById(
	"blocked-channels-nav",
) as HTMLLIElement;
const blockedTitlesNav: HTMLLIElement = document.getElementById(
	"blocked-titles-nav",
) as HTMLLIElement;
const blockedNamesNav: HTMLLIElement = document.getElementById(
	"blocked-names-nav",
) as HTMLLIElement;
const blockedCommentsNav: HTMLLIElement = document.getElementById(
	"blocked-comments-nav",
) as HTMLLIElement;
const excludedChannelsNav: HTMLLIElement = document.getElementById(
	"excluded-channels-nav",
) as HTMLLIElement;
const appearanceNav: HTMLLIElement = document.getElementById(
	"appearance-nav",
) as HTMLLIElement;
const importExportNav: HTMLLIElement = document.getElementById(
	"import-export-nav",
) as HTMLLIElement;
const aboutNav: HTMLLIElement = document.getElementById(
	"about-nav",
) as HTMLLIElement;
const faqNav: HTMLLIElement = document.getElementById(
	"faq-nav",
) as HTMLLIElement;
...
export function loadDataFromStorage() {
	cbSettingsClient.fetchSettings(env.id).then((result) => {
		...
		if (storageObject.version === "0") {
			...
			for (
				let index = 0;
				index < storageObject.blockedChannels.length;
				index++
			) {
			...
			for (
				let index = 0;
				index < storageObject.excludedChannels.length;
				index++
			) {
...
function updateRulesUI() {
	...
	blockedChannelsSelect.classList.toggle(
		"larger",
		blockedChannelsSelect.childElementCount > 4,
	);
	blockedChannelsSelect.classList.toggle(
		"largest",
		blockedChannelsSelect.childElementCount > 8,
	);
}
...
(function initUI() {
	...
	blockedChannelsSelect.addEventListener("change", (event) => {
		blockedChannelsRemoveBtn.classList.toggle(
			"outlined",
			blockedChannelsSelect.value === "",
		);
	});
